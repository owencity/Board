	  ...
	  <div class="panel panel-default">
	    ...
	    <div class="panel-body">
	 		<form name="frm" action="${contextPath}/memUpdate.do" method="post">
	 			<input type = "hidden" id = "memUserid" name = "memUserid" value="${memverVo.memUserid}"/>
				...
				<table class="table table-bordered" style="text-align: center; border: 1px solid #ddddd";>
					...
					<tr>
						...
						<td colspan="2"><input id="memName" name="memName" class="form-control" type="text" maxlength="20" placeholder="이름을 입력하세요" value="${memberVo.memName}"/></td>
					</tr>
					<tr>
						<td style="width:110px; vertical-align: middle;">나이</td>
						<td colspan="2"><input id="memAge" name="memAge" class="form-control" type="text" maxlength="20" placeholder="나이를 입력하세요" value="${memberVo.memAge}"/></td>
					</tr>
					<tr>
						...
						<td colspan="2">
							...
							<label class="btn btn-primary <c:if test="${memberVo.memGender eq '남자' }">active</c:if>">
	   						 <input id="memGender" name="memGender" type="radio" autocomplete="off" value="남자" 
	   						 <c:if test="${memberVo.memGender eq '남자' }">checked</c:if> />남자
							</label>
	
							<label class="btn btn-primary <c:if test="${memverVo.memGender eq '여자' }">active</c:if>">
	    					<input id="memGender" name="memGender" type="radio" autocomplete="off" value="여자"
	    					<c:if test="${memverVo.memGender eq '여자' }">checked</c:if> />여자
							</label>
								</div>
							</div>
						</td>
					...
					<tr>
						...
						<td colspan="2"><input id="memEmail" name="memEmail" class="form-control" type="text" maxlength="20" placeholder="이메일을 입력하세요" value="${memberVo.memEmail}"/></td>
					</tr>
					<tr>
